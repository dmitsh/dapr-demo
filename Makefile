...
build:
	mkdir -p bin
	cd bin; for f in $(SRCS); do echo $$f; CGO_ENABLED=0 go build -a -ldflags '-extldflags "-static"' ../$$f; done

build-linux:
	mkdir -p bin/linux
	cd bin/linux; for f in $(SRCS); do echo $$f; CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -ldflags '-extldflags "-static"' ../../$$f; done
